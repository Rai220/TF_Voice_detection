...
public class SpeechActivity extends Activity {
  ...
  private volatile long lastLabel = 0;
    ...
    while (shouldContinueRecognition) {
      ...
      runOnUiThread(
          new Runnable() {
            @Override
            public void run() {
              if (System.currentTimeMillis() - lastLabel > 1000) {
                // If we do have a new command, highlight the right list entry.
                Log.i("Kostya_recognizer", result.foundCommand + " " + result.score);
                TextView tv = (TextView) findViewById(R.id.statusText);
                if (result.foundCommand.equals("_silence_")) {
                  tv.setText("Тишина");
                } else {
                  tv.setText("Речь");
                }
                ...
                if (!result.foundCommand.startsWith("_") && result.isNewCommand) {
                  ...
                  for (int i = 0; i < labels.size(); ++i) {
                    if (labels.get(i).equals(result.foundCommand)) {
                      labelIndex = i;
                      tv.setText("Речь: " + result.foundCommand);
                      lastLabel = System.currentTimeMillis();
                      labelIndex = 0;
                    }
                  }
